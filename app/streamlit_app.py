...
with st.form("prediction_form"):
    ...
    with col1:
        gender = st.radio("Cinsiyet (Gender)", ["Male", "Female"], horizontal=True)
        age = st.slider("Yaş (Age)", min_value=1, max_value=120, value=35)
    with col2:
        customer_type = st.radio("Müşteri Tipi", ["Loyal Customer", "disloyal Customer"], horizontal=True)
        flight_distance = st.slider("Uçuş Mesafesi", min_value=10, max_value=15000, value=800, step=50)
    with col3:
        type_of_travel = st.radio("Seyahat Sebebi", ["Personal Travel", "Business travel"], horizontal=True)
    with col4:
        customer_class = st.radio("Sınıf (Class)", ["Eco", "Eco Plus", "Business"], horizontal=True)
    ...
    with col_d1:
        departure_delay = st.slider("Kalkış Gecikmesi (Departure Delay)", 0, 1000, 0, step=5)
    with col_d2:
        arrival_delay = st.slider("Varış Gecikmesi (Arrival Delay)", 0, 1000, 0, step=5)
